            ...
            <div class="col-lg-11">
                <div class="card shadow-lg p-4 ms-4">
                    ...
                    @if (in_array(session('role_name'), ['Admin', 'Superuser']))
                        <form id="taskForm" action="{{ route('job-task-update', $task->id) }}" method="POST"
                            ...
                            @if ($task->jobTaskAssign && $task->jobTaskAssign->jobExecutiveTask)
                                ...
                                <div class="row align-items-center mt-4">
                                    ...
                                    <div class="col-md-8">
                                        <textarea id="remarks" class="form-control" rows="4" readonly>{{ $task->jobTaskAssign->jobExecutiveTask->remarks }}</textarea>
                                    </div>
                                </div>
                    ...
                    @if (in_array(session('role_name'), ['Accounts', 'PR', 'HR', 'R&D']))
                        ...
                        @if (!$task->jobTaskAssign || !$task->jobTaskAssign->jobExecutiveTask)
                            <form id="taskForm" action="{{ route('job-task-executive') }}" method="POST"
                                class="container">
                                @csrf
                        ...
                        @if ($task->jobTaskAssign && $task->jobTaskAssign->jobExecutiveTask)
                            <form id="taskForm"
                                ...
                                <div class="row align-items-center mt-4">
                                    ...
                                    <div class="col-md-8">
                                        <textarea class="form-control fw-bold text-dark" rows="4" name="remarks">{{ $task->jobTaskAssign->jobExecutiveTask->remarks }}</textarea>
                                    </div>
                                </div>
