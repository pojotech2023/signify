            ...
            <div class="col-lg-11">
                <div class="card shadow-lg p-4 ms-4">
                    ...
                    @if (in_array(session('role_name'), ['Admin', 'Superuser']))
                        <form id="taskForm" action="{{ route('task-update', $task->id) }}" method="POST"
                            ...
                            @if ($task->leadTaskAssign && $task->leadTaskAssign->leadExecutiveTask)
                                ...
                                <div class="row align-items-center mt-4">
                                    ...
                                    <div class="col-md-8">
                                        <textarea id="remarks" class="form-control" rows="4" readonly>{{ $task->leadTaskAssign->leadExecutiveTask->remarks }}</textarea>
                                    </div>
                                </div>
                    ...
                    @if (in_array(session('role_name'), ['Accounts', 'PR', 'HR', 'R&D']))
                        ...
                        @if (!$task->leadTaskAssign || !$task->leadTaskAssign->leadExecutiveTask)
                            <form id="taskForm" action="{{ route('task-executive') }}" method="POST"
                                class="container">
                                @csrf
                        ...
                        @if ($task->leadTaskAssign && $task->leadTaskAssign->leadExecutiveTask)
                            <form id="taskForm"
                                ...
                                <div class="row align-items-center mt-4">
                                    ...
                                    <div class="col-md-8">
                                        <textarea class="form-control" rows="4" name="remarks">{{ $task->leadTaskAssign->leadExecutiveTask->remarks }}</textarea>
                                    </div>
                                </div>
